...
function Navbar() {
    ...
    return (
        <nav className="border shadow-sm w-full z-10 top-0 left-0 sticky">
                <div className="bg-white w-full top-0 sticky left-0">
                    ...
                    <img src="" alt="logo" className="flex items-start justify-start mr-auto"  /> {/* //añadir logo */}
                    <ul className="text-[#1DBF73] flex flex-row gap-3 p-2 left-0 z-auto font-medium">
                        {Links.map((link) => (
                        <li key={link.link} className="">
                            <Link to={link.link} className="flex p-2 items-center text-[#404145] hover:rounded hover:text-[#19A463]">
                                {link.name}
                            </Link>
                        </li>
                        ))}
                        <button className="items-center border border-[#19A463] text-[#19A463] rounded-[5px_5px_5px_5px] md:ml-8 w-16 hover:bg-[#19A463] hover:text-white"> Join </button>
                    </ul>
                    </div>
